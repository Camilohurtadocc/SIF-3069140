...
import { createError, createPaginatedResponse } from '../utils/errorHelper.js';
    ...
    try {
      ...
      const whereClause = search ? {
        [Op.or]: [
          ...
          { idioma: { [Op.like]: `%${search}%` } }
        ]
      } : {};

      // Get users with their roles using raw query
      const usersQuery = `
        ...
        ${search ? `WHERE u.correo_electronico LIKE '%${search}%' OR u.idioma LIKE '%${search}%'` : ''}
  ...
  static async updateWithRole(req, res) {
    ...
    try {
      ...
      res.json({
        ...
        user: {
          ...userData,
          roles: roles  // ✅ Incluir roles actualizados
        },
