...
export default function RecibosDeCaja() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch('http://localhost:3001/api/v1/recibo-caja', {
        method: 'POST',
        headers: {
      ...
      if (!response.ok) {
        ...
        console.error('Error del servidor (Status 400):', {
      ...
      console.error('Error creando recibo:', error.message)
      console.error('Detalles completos:', {
        message: error.message,
        stack: error.stack,
        payload: recibo
      })
      toast.error(error.message || 'Error al crear el recibo')
    } finally {
      setLoading(false)
    }
  }
